...
type RpaLog = { timestamp: number | string; type: "stdout" | "stderr"; message: string };
type ServerSms = {
...
export default function UnifiedHistoryPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(local)) {
          for (const it of local) {
            ...
            const title = `📱 ${it.phone || "-"} — ${String(it.message || "").trim()}`;
            const level =
              it.status === "success" ? "success" :
              it.status === "failed" ? "failed" :
              it.status === "error" ? "error" : "info";
            unified.push({
              id: `sms-local-${ts}-${it.phone || ""}-${Math.random().toString(36).slice(2,7)}`,
              timeEpoch: ts,
              timeText: fmtJa(ts),
              kind: "sms",
              level,
      ...
      try {
        ...
        const data = await resp.json();
        if (!data?.success) throw new Error(data?.error || "ログ取得に失敗しました");
  ...
  function exportCSV() {
    const rows = [
      ...
      ...view.map((x) => [x.timeText, x.kind, x.level, x.title, (x.detail || "").replace(/\r?\n/g, " ")]),
    ];
    const csv = rows
  ...
  return (
    <>
      <div className="panel-header" style={{ marginBottom: 16 }}>
        <h2 className="panel-title" style={{ color: brand, margin: 0 }}>📜 実行履歴</h2>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 12 }}>
        <select
          ...
          style={{ padding: "8px 10px", borderRadius: 8, border: "2px solid #e8eae0", background: "#fff" }}
          title="種類でフィルター"
        >
        ...
        <input
          ...
          style={{ flex: "1 1 260px", padding: 10, borderRadius: 8, border: "2px solid #e8eae0", background: "#fafbf7", color: "#43503a" }}
        />
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <select
            ...
            title="RPA 取得件数"
          >
            {[200, 500, 1000].map(n => <option key={n} value={n}>RPA {n}件</option>)}
          </select>
          <button
            onClick={refresh}
            style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #e6e8d9", background: "#fff", cursor: "pointer" }}
          >
            🔄 取得
          </button>
          <button
      ...
      {/* 列表 */}
      <div style={{ background: "#fff", border: "1px solid #e6e8d9", borderRadius: 12, padding: 12 }}>
        {err && <div style={{ color: "#d32f2f", marginBottom: 8 }}>❌ {err}</div>}
        {loading ? (
          <p style={{ color: "#666" }}>読み込み中...</p>
        ) : view.length === 0 ? (
          <p style={{ color: "#777", textAlign: "center", margin: "12px 0" }}>履歴がありません</p>
        ) : (
              ...
              <thead>
                <tr style={{ background: "#f6f7f2" }}>
                  {["時刻", "種類", "状態", "概要", "詳細"].map((h) => (
                    <th key={h} style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #e6e8d9", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {view.map((x) => {
                  const color =
                    x.level === "success" ? "#388e3c" :
                    x.level === "failed"  ? "#d32f2f" :
                    x.level === "error"   ? "#ff9800" : "#666";
                  const kindText = x.kind === "sms" ? "個別送信" : "RPA";
                  return (
                    <tr key={x.id} style={{ borderBottom: "1px solid #f0f0f0" }}>
                      <td style={{ padding: 10, whiteSpace: "nowrap" }}>{x.timeText}</td>
                      <td style={{ padding: 10, whiteSpace: "nowrap" }}>{kindText}</td>
                      <td style={{ padding: 10, whiteSpace: "nowrap", color, fontWeight: 700 }}>
                        {x.level.toUpperCase()}
                      </td>
                      <td style={{ padding: 10, minWidth: 240 }}>{x.title}</td>
                      <td style={{ padding: 10, color: "#555", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                        {x.detail || "-"}
                      </td>
                    </tr>
                  );
