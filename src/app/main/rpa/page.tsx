...
export default function RpaPage() {
  ...
  async function runChecks() {
    ...
    try {
      ...
      const cfg = api?.getUserConfig ? await api.getUserConfig() : await getUserConfigFallback();
      ...
      const appPwdOk                = !!appPwd && appPwd.length === 16 && /^[A-Za-z0-9]{16}$/.test(appPwd);
      ...
      const updated: CheckItem[] = [
        { key: "emailFormat",  label: "📧 メール形式",                 pass: emailFormatOk,         hint: emailFormatOk ? "" : "メール形式を確認してください" },
        { key: "targetMailbox",label: "📬 RPA対象メールボックス",       pass: targetMailboxNonEmpty, hint: targetMailboxNonEmpty ? "" : "必須です（空白不可）" },
        { key: "appPwd",       label: "🔑 Gmailアプリパスワード(16桁)", pass: appPwdOk,              hint: appPwdOk ? "" : "空白を除去し16桁の英数字で入力" },
        { key: "sitePwd",      label: "🌐 Indeedログインパスワード",     pass: sitePwdOk,             hint: sitePwdOk ? "" : "必須です（空白不可）" },
        { key: "apiUrl",       label: "🌐 SMS API URL",                pass: apiUrlOk,              hint: apiUrlOk ? "" : "必須です（空白不可）" },
        { key: "apiId",        label: "🆔 SMS API ID",                 pass: apiIdOk,               hint: apiIdOk ? "" : "必須です（空白不可）" },
        { key: "apiPwd",       label: "🔐 SMS API パスワード",          pass: apiPwdOk,              hint: apiPwdOk ? "" : "必須です（空白不可）" },
        { key: "server",       label: "🖥️ サーバー連携( /api/health )",  pass: serverPass,            hint },
      ];
      setChecks(updated);
      setStatusText(updated.every(x => x.pass) ? "✅ すべての前提条件を満たしました" : "⚠️ 未完了の設定があります");
    } catch (e: any) {
  ...
  function openLogs() {
    if (!userUid) return;
    window.open(`/api/rpa/logs/${userUid}?limit=200`, "rpaLogs", "width=900,height=700,scrollbars=yes");
  }
  ...
  return (
    <>
      <div className="panel-header" style={{ marginBottom: 16 }}>
        <h2 className="panel-title" style={{ color: "#6f8333", margin: 0 }}>🛠️ RPA実行</h2>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <h3 style={{ margin: 0, color: "#8c9569", fontSize: "1.05rem" }}>✅ 前提チェック</h3>
          <div>
            <a href="/main/account" style={{ marginRight: 8, fontSize: 13, color: "#6f8333", textDecoration: "underline" }}>
              アカウント設定へ
            </a>
            <a href="/main/sms" style={{ fontSize: 13, color: "#6f8333", textDecoration: "underline" }}>
              SMS設定へ
            </a>
          </div>
        </div>
        ...
        <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
          {checks.map((c) => {
            const map: any = {
              true:  { icon: "✅", color: "#388e3c", bg: "#e8f5e8", border: "#a5d6a7" },
              false: { icon: "❌", color: "#d32f2f", bg: "#ffeaea", border: "#ef9a9a" },
              null:  { icon: "⏳", color: "#888",    bg: "#f5f5f5", border: "#ddd" },
            };
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: c.pass ? "#777" : "#d32f2f" }}>
                  {c.hint || (c.pass ? "OK" : "")}
                </div>
              </li>
            );
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              fontWeight: 700,
              cursor: (!allPass || loading || running) ? "not-allowed" : "pointer",
          ...
          <button
            type="button"
            onClick={openLogs}
            style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #e6e8d9", background: "#fff" }}
          >
            📜 ログを開く
          </button>
        </div>
...
function renderStatus(state: string, data: any) {
  ...
  if (data?.startTime) parts.push(`開始: ${new Date(data.startTime).toLocaleString("ja-JP")}`);
  if (data?.endTime)   parts.push(`終了: ${new Date(data.endTime).toLocaleString("ja-JP")}`);
  if (Number.isFinite(data?.logCount)) parts.push(`ログ件数: ${data.logCount}件`);
  return parts.join(" / ");
}
